...
class Completion
  def self.generate
    new.tap { |completion| yield completion }.generate
  end

  def initialize(command: nil, description: nil)
    @command = command
    @description = description
    @condition = []
    @short_options = []
...
class Switch
  ...
  def initialize(shorts, longs, description, metavar)
    @completion = Completion.new(description: description)
...
class DockerCmdLine
  ...
  def parse_switch(line)
    ...
    Switch.new(shorts, longs, description, metavar)
  end
...
class DockerFishGenerator < BaseFishGenerator
  ...
  def function(commands)
    f = <<~FUNCTION
      function _halostatue_fish_docker_no_subcommand --description 'Test if docker has yet to be given the subcommand'
